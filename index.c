...
/*
Halfling Generator - Labyrinth Lord
...
char* primeReqBonus(unsigned dexterity, unsigned constitution);
...
int main(void)
{
	/*Holds the halfling's name*/
	char halflingName[ARRAY_SIZE];

	/*Holds the halfling's alignment*/
	...
	/*Stores halfling class bonuses*/
	char halflingBonuses[ARRAY_SIZE] = "\n90% chance to hide in bushes or outdoor cover.\n2 in 6 chance to hide in shadows or behind cover underground.\n+1 bonus to missile attacks";

	/*Save file name*/
	char saveFile[ARRAY_SIZE] = "";

	/*Introduction text*/
	printf_s("Welcome to the Labyrinth Lord halfling Generators.\n", ARRAY_SIZE);

	/*prompt user to enter halfling's name*/
	printf_s("\nEnter the name of your halfling: \n", ARRAY_SIZE);

	scanf_s("%[^\n]s", halflingName, ARRAY_SIZE);

	/*displays the name user entered*/
	printf_s("\nThe name you entered is: %s\n", halflingName, ARRAY_SIZE);
	...
	do {
		...
		/*if statement used to ensure dexterity is minimum of 9 as halfling
		class requirement*/
		if (dexterity < 9)
		{
			dexterity = 9;
		...
		/*if statement used to ensure constitution is minimum of 9 as halfling
		class requirement*/
		if (constitution < 9)
		{
			constitution = 9;
		...
		rangeTHACO = thaco(dexModifier) - 1;


		strcpy(primeRequisiteBonus, primeReqBonus(dexterity, constitution));
		...
		printf_s("\nNotes: %s%s%s", primeRequisiteBonus, charismaReaction, halflingBonuses, ARRAY_SIZE);
	...
	printf_s("\nYou have %u gold pieces (gp) to spend on your halfling.  Now it is time to buy some armour, weapons and gear.\n\n", gold, ARRAY_SIZE);
	...
	/*Option of purchasing a shield if halfling has at least 10 gp*/
	if (gold >= 10)
	{
	...
	printf_s("\nNow, let's purchase some weapons for your halfling.\n", ARRAY_SIZE);

	/*Function for adding weapons for halfling*/
	do
	{
		choice = 0;
		...
		printf_s("1. Short Sword      7 gp     1d6\n", ARRAY_SIZE);
		...
		case 1:
			if (gold > 7)
			{
				expense = 7;
				gold -= expense;
				strcat(weapons, "Short Sword");
				printf_s("\nYou have purchased a Short Sword\n", ARRAY_SIZE);
			}
			else
			{
				printf_s("\nYou do not have enough gold for the Short Sword\n", ARRAY_SIZE);
			}
	...
	/*Function for adding gear for halfling*/
	do
	{
		choice = 0;
	...
	printf_s("\nHere is an update of your Halfling:\n", ARRAY_SIZE);
	...
	printf_s("\nDo you want to save your halfling (%s) to a text files?\n1.yes\n2.no\n(please select 1 (yes) or 2 (no)\n", halflingName, ARRAY_SIZE);

	scanf(" %d", &noYes);

	if (noYes == 1)
	{
		printf_s("\nPlease a name (no spaces, please) for the file you want to save the halfling's information to: ", ARRAY_SIZE);
...
/*Randomly generates the halfling's hit points
...
/*displays the prime requisite bonus for halfling*/
char* primeReqBonus(unsigned dexterity, unsigned constitution)
{
	char primeReq[ARRAY_SIZE] = " ";

	if (dexterity < 13 && constitution >= 13)
	{
		strcpy(primeReq, "\n+5% Experience Adjustment");
	}
	else if (dexterity >= 13 && constitution < 13)
	{
		strcpy(primeReq, "\n+5% Experience Adjustment");
	}
	else if (dexterity >= 13 && constitution >= 13)
